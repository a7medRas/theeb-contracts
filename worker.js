// Cloudflare Worker: dynamic CORS + /ping + accepts multipart/form-data + auto-create folder
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && url.pathname === '/ping') {
      return new Response(JSON.stringify({ ok:true, msg:'pong' }), { status: 200, headers: { 'Content-Type':'application/json', ...cors } });
    }
    ...
    try {
      ...
      const qName  = url.searchParams.get('name') || `contract-${Date.now()}.pdf`;

      let fileName = qName, fileBuf=null;
      const ct = request.headers.get('Content-Type') || '';
      if (ct.includes('multipart/form-data')) {
        const form = await request.formData();
        const file = form.get('file');
        if (!file) throw new Error('Missing form field: file');
        fileName = file.name || qName;
      ...
      await ensureFolder(access, folder);
